...
PassForm::PassForm(Person* person, QWidget* parent) : QDialog(parent) {
    printed = false;
    ...
    photoLabel->setText("\U0001F464"); // Иконка человека
    ...
    if (rup != nullptr) {
        QString patronymic = (rup->patronymic == "-") ? "" : rup->patronymic;
        QString text = "<b>Фамилия:</b> " + rup->lastName + "<br>"
                       + "<b>Имя:</b> "      + rup->firstName + "<br>"
                       + "<b>Отчество:</b> " + patronymic + "<br>"
                       + "<b>Дата:</b> "     + rup->getDateRu();
        infoLabel->setText(text);
    } else if (amp != nullptr) {
        QString text = "<b>Имя:</b> "      + amp->firstName + "<br>"
